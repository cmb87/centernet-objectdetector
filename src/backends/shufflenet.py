...
def Shuffle_Net(start_channels, groups = 2,  nf=256, input_shape = (224,224,3)):
    ...
    # ============================================
    if True:
        upsample1 = upsample(x32, interpolation="bilinear")
        #upsample1 = convolution(upsample1,filters=nf,k=3,s=1)
        upsample1 = fire_module(upsample1, nf//2, nf)
        x = convolution(x16,filters=nf,k=1,s=1)
        x = Add()([upsample1, x])
        
        upsample2 = upsample(x, interpolation="bilinear")
        #upsample2 = convolution(upsample2,filters=nf,k=3,s=1)
        upsample2 = fire_module(upsample2, nf//2, nf)
        x = convolution(x8,filters=nf,k=1,s=1)
        x = Add()([upsample2, x])

        upsample3 = upsample(x, interpolation="bilinear")
        #upsample3 = convolution(upsample3,filters=nf,k=3,s=1)
        upsample3 = fire_module(upsample3, nf//2, nf)
        x = convolution(x4,filters=nf,k=1,s=1)
        x = Add()([upsample3, x])
      #  x = upsample3
