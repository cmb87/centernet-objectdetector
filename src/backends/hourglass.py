...
def hourglass_module(x, num_classes, num_channels, bottleneck, hg_id, dims):
    """
    ...
    downsample_features = create_downsample_blocks(x, bottleneck, hg_id, dims)

    # create upsample features, and connect with downsample features
    upsample_feature = create_upsample_blocks(downsample_features, bottleneck, hg_id, dims)
...
def create_downsample_blocks(x, bottleneck, hg_id, dims):
    """
    ...
    downsample_f1 = bottleneck(x, dims[0], hg_name + '_downsample_1')
    _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(downsample_f1)

    downsample_f2 = bottleneck(_x, dims[1], hg_name + '_downsample_2')
    _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(downsample_f2)

    downsample_f4 = bottleneck(_x, dims[2], hg_name + '_downsample_4')
    _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(downsample_f4)

    downsample_f8 = bottleneck(_x, dims[3], hg_name + '_downsample_8')

    return (downsample_f1, downsample_f2, downsample_f4, downsample_f8)



def create_upsample_blocks(downsample_features, bottleneck, hg_id, dims):
    """
    ...
    upsample_f8 = bottom_block(downsample_f8, bottleneck, hg_id, dims[0])
    upsample_f4 = connect_downsample_upsample(downsample_f4, upsample_f8, bottleneck, dims[1], hg_name+'_upsample_f4')
    upsample_f2 = connect_downsample_upsample(downsample_f2, upsample_f4, bottleneck, dims[2], hg_name+'_upsample_f2')
    upsample_f1 = connect_downsample_upsample(downsample_f1, upsample_f2, bottleneck, dims[3], hg_name+'_upsample_f1')
...
def connect_downsample_upsample(downsample_feature, upsample_feature, bottleneck, num_channels, name):
    ...
    """
    nChannels = upsample_feature.shape[-1]

    downsample_x = bottleneck(downsample_feature, nChannels, name + '_short')
...
def get_hourglass_model(num_classes, num_stacks, num_channels, model_input_shape=None, mobile=False, dims=4*[256]):
    ...
    for i in range(num_stacks):
        head_next_stage, head_to_loss = hourglass_module(head_next_stage, num_classes, num_channels, bottleneck, i, dims)
...
if __name__ == "__main__":
    ...
    model = get_hourglass_model(num_classes, num_stacks, num_channels, model_input_shape=model_input_shape, mobile=False, dims=[256,312,312,512])
    print(model.summary())

# https://github.com/david8862/tf-keras-stacked-hourglass-keypoint-detection/blob/master/hourglass/data.py