...
def centerNetLoss(ytrue, ypred, alpha=2.0, beta=3.0):
...
def cls_loss(ytrue, ypred, alpha=2.0, beta=4.0):
    C = tf.shape(ytrue)[-1] - 4
    hmTrue, _, _ = tf.split(ytrue, [C, 2, 2], axis=-1)
    hmPred, _, _ = tf.split(ypred, [C, 2, 2], axis=-1)
    hmPred = tf.clip_by_value(hmPred, 1e-4, 1.0 - 1e-4)
    ...
    lossHm = -tf.reduce_sum(m*a + (1.0-m)*b, axis=[1,2,3])
    return tf.reduce_mean(1.0/N * lossHm)


def reg_loss(ytrue, ypred):
    C = tf.shape(ytrue)[-1] - 4
    _, whLogTrue, pdeltaTrue = tf.split(ytrue, [C, 2, 2], axis=-1)
    _, whLogPred, pdeltaPred = tf.split(ypred, [C, 2, 2], axis=-1)
    ...
    return tf.reduce_mean(1.0/N_box * (0.1 * lossWh + lossPdelta))


def wh_loss(ytrue, ypred):
    C = tf.shape(ytrue)[-1] - 4
    _, whLogTrue, pdeltaTrue = tf.split(ytrue, [C, 2, 2], axis=-1)
    _, whLogPred, _ = tf.split(ypred, [C, 2, 2], axis=-1)

    mask = tf.expand_dims(tf.cast(tf.greater(pdeltaTrue[...,0],0.0), tf.float32), -1) # [B,H,W,1]
    N_box = tf.reduce_sum(mask, axis=[1,2,3]) + 1.0 

    lossWh = tf.reduce_sum( mask*tf.math.abs(whLogPred - whLogTrue), axis=[1,2,3])
    return tf.reduce_mean(1.0/N_box * lossWh)


def offset_loss(ytrue, ypred):
    C = tf.shape(ytrue)[-1] - 4
    _, _, pdeltaTrue = tf.split(ytrue, [C, 2, 2], axis=-1)
    _, _, pdeltaPred = tf.split(ypred, [C, 2, 2], axis=-1)

    mask = tf.expand_dims(tf.cast(tf.greater(pdeltaTrue[...,0],0.0), tf.float32), -1) # [B,H,W,1]
    N_box = tf.reduce_sum(mask, axis=[1,2,3]) + 1.0 

    lossPdelta = tf.reduce_sum( mask*tf.math.abs(pdeltaPred - pdeltaTrue), axis=[1,2,3])
    return tf.reduce_mean(1.0/N_box * lossPdelta)
